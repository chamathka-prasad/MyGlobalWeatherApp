...
function turnOn() {
    ...
    setInterval(exTractData(true), 60000 * 5);
}
function getDataFromOpenWeather(array, statBoolean) {
    ...
    async function filterData() {
        ...
        aryNew.push(weather);
        localStorage.setItem("city", JSON.stringify(aryNew));
...
function fullView(cit_id) {
    ...
    function looper() {
        ...
        setInterval(exTractData(false), 60000 * 5);

        var tl = document.getElementById("tableLoad");
        tl.innerHTML = " ";
        ...
        for (let i = 0; i < js.length; i++) {
            ...
            for (let j = 0; j < js[i].length; j++) {

                if (js[i][j].id == cit_id) {
                    
                    if (i == js.length - 1) {
                        document.getElementById("country").innerHTML = js[i][j].name + "," + js[i][j].sys.country;


                        var d = new Date(js[i][j].dt * 1000);
                        ...
                        document.getElementById("desc").innerHTML = js[i][j].weather[0].description;



                        document.getElementById("tem").innerHTML = js[i][j].main.temp + " &#8451";
                        document.getElementById("tem_min").innerHTML = js[i][j].main.temp_min + " &#8451";
                        document.getElementById("tem_max").innerHTML = js[i][j].main.temp_max + " &#8451";

                        document.getElementById("press").innerHTML = js[i][j].main.pressure + "hPa";
                        document.getElementById("hum").innerHTML = js[i][j].main.humidity + "%";
                        document.getElementById("vis").innerHTML = (js[i][j].visibility / 1000) + "km";



                        var sunR = new Date(js[i][j].sys.sunrise * 1000);

                        document.getElementById("sr").innerHTML = sunR.getHours() + "." + sunR.getMinutes() + " " + sunR.toLocaleTimeString().split(" ")[1];

                        var sunS = new Date(js[i][j].sys.sunset * 1000);

                        document.getElementById("ss").innerHTML = sunS.getHours() + "." + sunS.getMinutes() + " " + sunS.toLocaleTimeString().split(" ")[1];

                        document.getElementById("win").innerHTML = js[i][j].wind.speed + "m/s " + js[i][j].wind.deg + " Degree";
                    }
                    ...
                    td2.innerHTML = js[i][j].main.pressure + "hPa" + "</br>" + js[i][j].main.humidity + "%" + "</br>" + (js[i][j].visibility / 1000) + "km";
                    tr.appendChild(td2);

                    var td3 = document.createElement("td");
                    td3.innerHTML = js[i][j].main.temp + " &#8451";
                    tr.appendChild(td3);

                    var td4 = document.createElement("td");
                    td4.innerHTML = js[i][j].weather[0].description;
                    tr.appendChild(td4);
